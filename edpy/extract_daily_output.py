...
def extract_ed2_daily(
    ...
    '''
        Extract daily level data, including ecosystem average vars and profile
    ...
    for iyear, year in enumerate(year_array):
        ...
        for var in col_list + voi_avg:
            output_dict[var] = np.zeros((366,))

        # PFT vars
        for var in voi_avg_pft:
            for pft in pft_list:
                output_dict['{:s}_PFT_{:d}'.format(var,pft)] = np.zeros((366,))

        # Size vars
        for var in voi_size:
            for isize, size_edge in enumerate(size_list[1]):
                output_dict['{:s}_{:s}_{:d}'.format(var,size_list[0],isize)] = np.zeros((366,))

        # size PFT vars
        for var in voi_pft_size:
            for pft in pft_list:
                for isize, size_edge in enumerate(size_list[1]):
                    output_dict['{:s}_PFT_{:d}_{:s}_{:d}'.format(var,pft,size_list[0],isize)] = np.zeros((366,))
        ...
        for month in month_array:
            ...
            for day in day_array:
                ...
                # append time stamps
                output_dict['year'][doy-1]  = year
                output_dict['month'][doy-1] = month
                output_dict['day'][doy-1]   = day
                output_dict['doy'][doy-1]   = doy

                # read data
                data_fn = '{:s}-D-{:4d}-{:02d}-{:02d}-000000-g01.h5'.format(
                    data_pf,year,month,day)
